...
import 'package:fpay/Network/Models/ReportApesResponse.dart';
...
class Aeps extends StatefulWidget {
  const Aeps({Key? key}) : super(key: key);

  @override
  State<Aeps> createState() => _AepsState();
}

class _AepsState extends State<Aeps> {
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text("APES", style: AppStyle.txtPoppinsBold18.copyWith()),
          centerTitle: true,
  ...
  Future checkOnBoarding() async {
    ...
    var jsonBody = await aepsController.onBoard(body);
    commonResponse = CommonResponse.fromJson(jsonBody);
    if (commonResponse?.message != "Complete") {
      Get.offNamed('/aepsOnBoarding');
    } else {
      outletStatus();
    }
  }

  Future outletStatus() async {
    var json = await aepsController.outletStatus();
    aepsLoginStatus = AepsLoginStatus.fromJson(json);
    if (aepsLoginStatus?.isCashWithdrawalLogin != true) {
      showLoginSheet();
    }
  }
  ...
  void scanFinger(String deviceName) async {
    if (Platform.isAndroid) {
      var arguments = {'device': deviceName};
      var data = await aepsChannel.invokeMethod('AEPSPIDDATA', arguments);
      setState(() {
        if (data.substring(0, 1) == "0") {
          outLetLogin(data.substring(2));
  ...
  Future outLetLogin(String pidData) async {
    var body = {"PidData": pidData};
    var json = await aepsController.outletLogin(body);
